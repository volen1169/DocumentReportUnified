...
def render_computer_asset(
    *,
    ...
    with _ca_nav[1]:
        if st.button("⏮️",use_container_width=True,key="ca_first",disabled=_ca_page<=1): st.session_state["ca_page"]=1; st.rerun()
    with _ca_nav[2]:
    ...
    with _ca_nav[4]:
        if st.button("▶️",use_container_width=True,key="ca_next",disabled=_ca_page>=_ca_page_count): st.session_state["ca_page"]=_ca_page+1; st.rerun()
    with _ca_nav[5]:
